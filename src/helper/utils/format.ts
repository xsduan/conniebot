 ...
 *
 * Property access takes two forms – `{foo.bar}` and `{foo[bar]}`. Top level
 * array access is not allowed because that's weird.
 *
...
export function strFormat(target: string, args: object) {
  return target.replace(
    ...
    (m, objPath) => {
      let curr: any = args;
      // technically there could be some odd sequences we allow here, but our
      // path regex ensures that it'll be sensible, at the very least.
...
/**
 * Perform `strFormat` on a plain object. (May work for prototyped objects,
 * but may be wonky, no guarantees.)
 *
